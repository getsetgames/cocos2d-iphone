...
void advance_gameswf()
{
    ...
    //m->set_display_viewport(0, 0, m->get_movie_width(), m->get_movie_height());
    //m->set_background_alpha(0);
...
void before_draw()
{
    GLfloat movieWidth = m->m_def->m_frame_size.m_x_max - m->m_def->m_frame_size.m_x_min;
    GLfloat movieHeight = m->m_def->m_frame_size.m_y_max - m->m_def->m_frame_size.m_y_min;
    
    CC_DISABLE_DEFAULT_GL_STATES();

    glTranslatef(0.0f, m->get_movie_height(), 0.0f);
    glScalef((m->get_movie_width() / movieWidth), -(m->get_movie_height() / movieHeight), 1.0f);
